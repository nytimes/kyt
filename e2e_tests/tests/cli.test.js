...
describe('KYT CLI', () => {
  ...
  it('this is a test', () => {
    const exec = new Promise((resolve) => {
      const child = shell.exec(`node_modules/.bin/kyt-cli setup`, (code) => {
      ...
      child.stdout.on('data', (data) => {
        if (data.includes('Would you like to setup with a starter-kyt?')) {
          if(!skdone) {
            child.stdin.write('y\n');
            skdone = true;
          }
        }
        if (data.includes('Which starter-kyt would you like to install')) {
          if(!chooseDone) {

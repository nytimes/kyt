...
describe("KYT CLI", () => {
  ...
  const packageScripts = () => {
    it("sets up the package json scripts", () => {
      const userPackageJSON = require.requireActual(
  ...
  const outputCheck = setupArr => {
    expect(
      setupArr.includes("👍  Added kyt scripts into your package.json scripts")
    ).toBe(true);
    expect(
      setupArr.includes("👍  Added new dependencies to package.json")
    ).toBe(true);
    ...
    it("sets up a universal starter-kyt", () => {
      const exec = new Promise(resolve => {
        const child = shell.exec(
          "../packages/kyt-cli/index.js setup",
          (code, stdout) => {

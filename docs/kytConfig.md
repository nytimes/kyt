...
## kyt.config.js options
 ...
 1. `serverURL` - url for the backend node server. *default*: http://localhost:3000. if `noServer` is set to `true`, then this value is ignored
 ...
 7. `noServer` - Turns off the backend node server for build and dev (useful if you already have a backend) *default*: false
